...
function MyJobPostingsScreen({ navigation }) {
  const { user } = useAuth();
  ...
  const fetchMyJobPostings = async () => {
    try {
      ...
      // Option 1: Requires composite index (recommended for performance)
      const options = {
        where: [{ field: "employerId", operator: "==", value: user.id }],
        orderBy: { field: "createdAt", direction: "desc" },
      };

      // Option 2: Alternative without index (less efficient, uncomment if needed)
      // const options = {
      //   where: [{ field: "employerId", operator: "==", value: user.id }],
  ...
  const renderJobCard = ({ item }) => (
    <View style={styles.jobCard}>
      ...
      <TouchableOpacity style={styles.editButton}>
        <Text style={styles.editButtonText}>Edit Posting</Text>
      </TouchableOpacity>
    </View>
  );
  ...
  return (
    <SafeAreaView style={[defaultStyles.container, styles.container]}>
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>Manage your posted jobs</Text>
      </View>
